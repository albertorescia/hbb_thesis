...
DIS is a process which investigates the insides of hadrons using leptonic probes. In this section we will focus on high-energy electron-proton inelastic scattering. In this process, the incoming electron exchanges a virtual electroweak boson with the proton. Specifically, we will look at the process
\begin{equation}
e^- \; p \rightarrow e^- \; X
\end{equation}
...
\label{kinematics}
\end{equation}
where $k^\mu$ and $k^{\mu\prime}$ refer to the four-momenta of the initial and final state electron, respectively, $p^\mu$ the four-momentum of the initial state proton and $q^\mu$ refers to the four-momentum of the virtual photon. The quantities $E$ and $E^\prime$ refer to the respective energies of the electron in the initial and final state, and $\theta$ is the angle at which the electron is scattered with respect to its initial momentum $\vec{k}$. 
...
The first kinematic variable to consider is the energy of the virtual probe, known also as the scale of the process. Since DIS is a space-like process, we have $q^2 < 0$, where $q^2$ represents the four-momentum of the virtual probe. Therefore we take
\begin{equation}
Q^2 = -q^2.
\end{equation}
...
\end{equation}
We can therefore identify Bjorken $x$ with the momentum fraction of the struck constituent quark.
...
The discussion up to now has focused exclusively on quarks, since the original framework of the so-called \emph{Quark Parton Model} did not include gluons. However, Figure \ref{PDF plot} shows that, particularly at small $x$, i.e.\ at high energies, a significant fraction of the proton's constituents are gluons.
...
\end{equation}
which in the TRF represents the fraction of energy of the observed particle lost in the hadronization process. 
...
In order to be able to precisely calculate jet cross sections and confront these with experimental data, it is necessarily to unambiguously \emph{define} what a jet is. There are multiple possible definitions of jets, and we will briefly illustrate some examples of these.  
...
A number of higher order corrections to the elementary cross section are possible. These include real and virtual corrections in all possible combinations. In QCD, we currently know how to treat corrections up to next-to-next-to-next-to-leading order for a few select processes \cite{DUHR20162128, Anastasiou:2015vya}. In this section we will briefly discuss the importance of higher order corrections, as well as some challenges which arise during their calculation.
...
\centering
\includegraphics[width=\textwidth]{ch2_images/real}
\label{real}
\caption{}
\end{subfigure}
\begin{subfigure}{.5\textwidth}
\centering
\includegraphics[width=\textwidth]{ch2_images/virtual}
\label{virtual}
\caption{}
\end{subfigure}
...
\end{equation}
which, when $s \gg m^2$, simplifies to
\begin{equation}
